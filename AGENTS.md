...
- **`CHANGELOG.md`** — *what* changed. One condensed line per change (`<date> · <type> — <headline>` + proof).
- **`DECISIONS.md`** — *why*, whenever you make or reverse an architectural/process choice
  (**Context → Options → Decision → Why → Status**).
- **`docs/sessions/<date>-<topic>.md`** — the *discussion*: what was explored and landed on (history,
  non-authoritative).
These three files are the project's memory; treat updating them as part of the change, not an afterthought.
A reviewer should be able to read the diff + the changelog line and understand the change without you in the room.

## File map
`index.html` (menu) · `js/engine.js` · `tools/*.html` · `docs/PACT-Players-Guide.html` ·
`testing/tests/engine-parity.html` + `testing/fixtures/` · **`CHANGELOG.md` · `DECISIONS.md` ·
`docs/sessions/` · `docs/PWA-BUILD-PLAN.md`** (the build roadmap).
