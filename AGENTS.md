...
> Sizes below measured **2026-09-10**. Re-measure rather than trust them if a decision hinges on the number —
> every figure in this list was once stale by 2–4× (see `D-GH-2026-07-29-file-review-4plpe3`), and the
> **2026-07-29** figures below were themselves stale by up to ~3× when re-measured on 2026-09-10 during a
> full-system audit — `js/engine.js` alone had drifted from a documented ~930 lines to an actual 2,660
> (2.9×). Stale-by-2-4× is not a one-off, it is what this section does between audits; re-measure, don't
> extrapolate from how recently it was last corrected.
- **`js/engine-data.js` (~206 KB, ~14 lines)** — the rules dataset `DATA`, split out of the engine
...
- **`js/engine.js` (~187 KB, ~2,660 lines)** — `grep` for the symbol you need (full API is the Exports line
  above). It has grown well past "small enough to read in full" since the last measurement — treat it like
  the tool files below and search within rather than reading it wholesale, unless the task is a deep engine
  refactor that genuinely needs the whole shape at once.
- **Never load `docs/PACT-Players-Guide.html` (~1.4 MB) or `assets/pact-cover.webp`** — player assets, not code.
- **`tools/*.html` are ~350–603 KB each** (DM Console ~382 KB, Live Sheet ~349 KB, CharGen ~603 KB) — search
