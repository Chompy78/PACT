...
- **2026-07-16 · feat(docs) — add `docs/dev-status.html`, a live-fetch glance dashboard**. Quick-glance
  human-status page: open Now/Next tasks + last 7 decisions + last 7 changelog entries, fetched live from
  `TASK_BOARD.md`/`CHANGELOG.md`/`DECISIONS.md` (never stale) and light-parsed — no Markdown library, no
  build step. Reuses `roadmap.html`'s palette (light + dark); distinct from and links to the fuller
  `roadmap.html`. Fetched text renders via `textContent` (escaping invariant); graceful `file://` fallback.
  Verified headless (Playwright): correct counts, 0 console errors, both themes. See `DECISIONS.md`
