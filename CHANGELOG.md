...
- **2026-09-10 · fix+chore: full-system audit — delete/save race, AP-grant dedupe, dead code, doc
  ...
  color and two raw-exception-text UI messages made theme-aware/friendlier; corrected AGENTS.md's and
  `js/engine.js`'s stale file-size/line-count figures (up to ~2.9× off — this section has now drifted
  stale three separate times, see its own updated warning); applied a purely-additive migration
  (`sql/migrations/2026-09-10-unindexed-foreign-keys.sql`) adding 11 covering indexes the Supabase
  performance advisor flagged, folded into `sql/schema.sql`, advisor re-checked clean afterward. Full
  session record: `docs/sessions/2026-09-10-full-system-audit.md`.
