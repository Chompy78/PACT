...
```text
1. OWNER DECISION FIRST: for a character starting above level 1, what ends creation? Candidates seen so
   far - (a) the threshold is that character's own creation budget rather than a flat DATA.level1AP,
   which is what the confirm prompt already collects; (b) an explicit "finish character" action;
   (c) the burst preserves each event's original noLock flag instead of blanket-tagging.
2. Note (c) conflicts with D5: the automatic lock is deliberately DERIVED and reversible, so that undo
   can un-fire it. Persisting a fired lock as an event turns it into a ratchet. Decide which property
   matters more and record it - they cannot both hold.
