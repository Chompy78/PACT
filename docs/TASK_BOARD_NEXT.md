...
## feat/keepalive-scope-narrowing: reconsider withKeepalive()'s widened scope from fix/autosave-flush-latest-push — TODO
Branch feat/keepalive-scope-narrowing. Found by `/code-review ultra` on commit `9616c22`, confirmed by
direct verification (not just the sub-agent's claim): both tools' `pagehide` handlers now wrap
`function(){ _cgCloudPush(); return _cgCloudPushSettled(); }` (and the `_ls*` twin in Live Sheet) in
`withKeepalive()`, instead of just the bare push call alone. `withKeepalive()` (`js/supabase-client.js`)
gates the shared Supabase client's fetch wrapper via one single, shared, mutable `_keepaliveNext` flag,
documented on the function itself as "scope it tightly (wrap exactly one call)". The widened wrap now
keeps that flag `true` for however long the settle-wait/retry chain takes, not just one push's own fetch
— so any other concurrent fetch through the same client during that window (most plausibly Supabase's own
background `autoRefreshToken` request, confirmed enabled in `js/supabase-client.js`) would silently also
get `keepalive:true`. See the Addendum on `decisions/2026/D-GH-2026-08-08-chargen-cloud-autosave-flush.md`
for the full context — that record's own "withKeepalive scope" paragraph explicitly assumed this window
stays narrow, which the 2026-08-10 fix reopened without resolving.
**Effort:** low · **Risk:** low — ambiguity is low (two concrete resolution shapes are already spelled out
below, not an open design question); damage scale is low (the exposure is one extra fetch-attribute flag
being true when it needn't be, not a functional/security bug — `keepalive:true` only affects whether the
browser lets an in-flight fetch outlive page unload); damage likelihood is low (`autoRefreshToken`'s
request has to land inside the exact pagehide settle-wait window to be affected at all, and this is a
`pagehide`-only code path, not a general-request-lifecycle bug) — safe for `/sweep-code-tasks`.

```text
1. Re-read the current pagehide handlers in both tools (`tools/PACT-CharGen-Webtool.html`,
   `tools/PACT-Live-Char-Sheet.html`) and js/supabase-client.js's `withKeepalive()` doc comment fresh —
   confirm the shape described above still matches (another task may have touched this file since).
2. Decide between the two options already identified, and implement whichever is chosen:
   - **Accept the widened window as a documented trade-off** — write the reasoning into the Addendum on
     `decisions/2026/D-GH-2026-08-08-chargen-cloud-autosave-flush.md` (why the exposure window is
     acceptable: bounded by the pagehide-only lifecycle, no known correctness impact).
   - **Narrow it back down** — wrap only the currently-in-flight push in `withKeepalive()`; let a chained
     retry (if `_cgCloudSaveAgain`/`_lsCloudSaveAgain` triggers one) fire its own separate, narrower
     `withKeepalive()` span rather than one span covering the whole settle-wait chain.
3. If narrowing: re-run `testing/scripts/autosave-flush-latest-push-ci.mjs` (must stay 8/8) since it
   exercises this exact pagehide/keepalive code path.
4. Update the Addendum on the decision record either way — recording the trade-off if accepted, or the
   narrower design if implemented — so the record's own "assumed narrow" paragraph no longer disagrees
   with shipped behavior.
```

**Done when:** the decision (accept vs. narrow) is recorded in the Addendum on
`decisions/2026/D-GH-2026-08-08-chargen-cloud-autosave-flush.md` with reasoning; if narrowed, both tools'
pagehide handlers only hold `withKeepalive()` open for the current push's own fetch span, and
`autosave-flush-latest-push-ci.mjs` stays 8/8.

## fix/manual-save-queue-bypass: route manual "Save to cloud"/"join campaign" saves through the push queue — TODO
Branch fix/manual-save-queue-bypass. Found by `/code-review ultra` on commit `9616c22`, confirmed by
direct verification: CharGen's `onSaveClick()` (`tools/PACT-CharGen-Webtool.html`, ~line 891) and
`onJoinCampaignClick()` (~line 943) both call `S.saveCharacter(...)` directly, bypassing
`_cgCloudSaveBusy`/`_cgCloudPush()` entirely — same pattern confirmed in Live Sheet's "☁ Save to cloud"
button handler (`tools/PACT-Live-Char-Sheet.html`, ~line 2432). This means a manual save can race the
autosave push queue's own in-flight state: if `_cgCloudPush()`/`_lsCloudPush()` already has a push
in-flight when the user clicks the manual save button, the two writes are not sequenced against each
other by the queue machinery that `fix/autosave-flush-latest-push` (this session, `D-GH-2026-08-08-
chargen-cloud-autosave-flush` Addendum) just hardened for every *other* save path. The clone-to-standalone
save in Live Sheet (~line 2559) mints a brand-new character id per save and is not part of this race —
scope this task to same-character saves only.
**Effort:** medium · **Risk:** low — ambiguity is low (the fix shape mirrors the autosave queue's own
established pattern — route through the existing `_cgCloudPush`/`_lsCloudPush` queue instead of calling
`S.saveCharacter` directly — not a new design); damage scale is low (a lost-update race on a single
character's own save, recoverable via the existing conflict-detection/reload flow already in these
handlers, not data corruption or cross-user exposure); damage likelihood is low (requires a manual save
click to land while an autosave push is already in flight — a real but narrow timing window, not
every-save).

```text
1. Re-read the current `onSaveClick()`/`onJoinCampaignClick()` in CharGen and the "☁ Save to cloud"
   handler in Live Sheet fresh (line numbers above are approximate; another task may have shifted them).
2. For each: replace the direct `S.saveCharacter(...)` call with a route through the tool's own
   `_cgCloudPush()`/`_lsCloudPush()` queue (setting whatever local state — LOG/SEQ/dirty flag — those
   functions expect as their input, matching how the tool's autosave path already primes that state
   before calling push), so a manual save and an in-flight autosave push are sequenced by the same
   `_cgCloudSaveBusy`/`_lsCloudSaveBusy` flag instead of racing each other.
3. Preserve each handler's own existing UI feedback (button disable/re-enable, conflict-dialog prompt,
   AP-budget-over-limit alert in Live Sheet's handler) — this is a plumbing change to how the save is
   dispatched, not a UI behavior change.
4. Extend `testing/scripts/autosave-flush-latest-push-ci.mjs` (or a new sibling script, if the manual-save
   entry points aren't reachable via that script's existing extraction helpers) with a scenario that
   proves a manual save no longer bypasses the queue: an autosave push in-flight, then a manual-save
   click, asserts the manual save waits on/joins the queue rather than firing a second uncoordinated
   `S.saveCharacter` call.
5. Run the full regression suite (JS syntax check, `autosave-flush-latest-push-ci.mjs`,
   `engine-parity-ci.mjs`, `tool-pricing-ci.mjs`, `sync-state-machine-ci.mjs`) before considering this done.
```

**Done when:** all three manual-save entry points (CharGen's two, Live Sheet's one same-character path)
route through the tool's own push-queue functions instead of calling `S.saveCharacter` directly; a new or
extended CI scenario proves a manual save no longer races an in-flight autosave push; the full regression
suite passes.
